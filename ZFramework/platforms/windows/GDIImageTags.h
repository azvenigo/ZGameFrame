...
std::string PropertyItemToString(PropertyItem* p)
{
    ...
    size_t nLength = p->length;
    if (nLength > 128)
        nLength = 128;

    switch (p->type)
    {
    case PropertyTagTypeASCII:      sType = "PropertyTagTypeASCII";     sValue = std::string((const char*)p->value, nLength);             break;
    case PropertyTagTypeByte:       sType = "PropertyTagTypeByte";      sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);   break;
    case PropertyTagTypeLong:       sType = "PropertyTagTypeLong";      sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);               break;
    case PropertyTagTypeRational:   sType = "PropertyTagTypeRational";  sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);               break;
    case PropertyTagTypeShort:      sType = "PropertyTagTypeShort";     sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);               break;
    case PropertyTagTypeSLONG:      sType = "PropertyTagTypeSLONG";     sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);               break;
    case PropertyTagTypeSRational:  sType = "PropertyTagTypeSRational"; sValue = StringHelpers::FromBin((uint8_t*)p->value, nLength);               break;
    case PropertyTagTypeUndefined:  sType = "PropertyTagTypeUndefined"; sValue = "";       break;
    }

    std::string sResult;
    Sprintf(sResult, "id:%s (0x%08d) type:%s length:%d value:%s", TagFromID(p->id).c_str(), p->id, sType.c_str(), p->length, sValue.c_str());
    return sResult;
}